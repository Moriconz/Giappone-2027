/**
 * Vercel Function: searchVintageShops
 * Search for vintage, second-hand, and thrift shops using Google Places API
...
export default async function handler(req, res) {
  ...
  async function searchVintageShops() {
    // Multiple search queries per coprire diverse categorie di vintage
    const queries = [
      `vintage shop ${city} Japan`,
      `second hand shop ${city} Japan`,
      `thrift store ${city} Japan`,
      `used clothing store ${city} Japan`,
      `retro shop ${city} Japan`
    ];
    ...
    for (const searchQuery of queries) {
      ...
      if (lat && lng) {
        ...
        params.set('radius', '15000'); // 15km radius
      }
      ...
      try {
          ...
          data.results.slice(0, 5).forEach(place => {
            if (!allShops.has(place.place_id)) {
              allShops.set(place.place_id, {
                ...
                is_vintage: true, // Flag per identificare vintage shops
        ...
        console.warn(`[searchVintageShops] Error with query "${searchQuery}":`, err.message);
      }
    }
