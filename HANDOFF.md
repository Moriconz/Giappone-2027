# HANDOFF — Tabi (Giappone 2027) · 2026-07-16 (fine sessione, v3.57)
...
## Stato attuale
- v3.54, v3.55, v3.56 committati e pushati su `main` (v3.54: `1db2f53`; v3.56 + fix guida: `9ffef18`, `1c1e844`). **v3.57 (quota condivisa server-side + toast + cache 2 anni) in attesa di commit/push** — vedi "Prossimi step".
...
- **Quota API multi-utente (v3.57)**: prima il limite giornaliero (`js/api-quota.js`, es. 15 ricerche/giorno) era per-dispositivo via localStorage — con N persone che usano l'app insieme la spesa reale poteva arrivare a N× il limite pensato per un utente solo. Ora c'è anche un contatore server-side condiviso (`api/_lib/quota.js`, stesso Redis/Upstash della cache), **ma funziona solo se un database Vercel KV è collegato al progetto** — altrimenti fail-open silenzioso (nessun blocco server, resta solo il guard per-dispositivo). Verificare su Vercel → Storage se KV è collegato; se non lo è, la protezione multi-utente reale non è ancora attiva in produzione.
...
- **Confermato dall'utente**: database `giappone-kv` (Upstash Redis Free) collegato al progetto, `KV_REST_API_URL`/`KV_REST_API_TOKEN`/`KV_URL`/`REDIS_URL` presenti su tutti gli ambienti — la quota condivisa e la cache condivisa sono attive, non serve altro setup.
...
1. **Commit + push v3.57** — quota condivisa server-side + toast + cache 2 anni (questa sessione). Eseguibile subito, nessuna decisione aperta.
2. **Verificare/collegare Vercel KV in produzione** — senza, la quota condivisa multi-utente (v3.57) resta fail-open e non protegge davvero dal costo N×utenti. Non è una scelta di scope, è un controllo da fare su Vercel → Storage.
