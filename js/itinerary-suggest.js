      ...
      const suggList = suggestions.length ? suggestions.map(s => `
        <div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-top:1px solid var(--l-hair);">
          ...
          <div style="flex:1;min-width:0;">
            <div style="font-size:15px;color:var(--l-ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${_esc(s.name)}</div>
            <div style="font-size:13px;color:var(--l-muted);">${s.dist.toFixed(1)} km${s.poi.rating ? ' · ⭐ ' + s.poi.rating : ''}${s.isGF ? ' · GF' : ''}</div>
          </div>
          <button data-sugg-add="${_esc(s.poi.id || s.poi.googlePlaceId)}" data-sugg-day="${day.dayIdx}" data-sugg-name="${_esc(s.name)}" data-sugg-lat="${s.poi.lat}" data-sugg-lng="${s.poi.lng}" style="
            ...
            border-radius:7px;color:#15803d;font-size:14px;font-weight:600;cursor:pointer;">➕ Day ${day.dayIdx + 1}</button>
        </div>
      `).join('') : `<div style="font-size:14px;color:var(--l-faint);padding:6px 0;">${hasFree ? T('sugg.noNearby', 'Nessun POI vicino non ancora in itinerario.') : T('sugg.dayFull', 'Giornata piena, poco tempo libero.')}</div>`;
      ...
      return `
        <div style="background:rgba(20,30,60,0.03);border:1px solid var(--l-hair);border-radius:11px;padding:12px 14px;">
          <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px;">
            <span style="font-weight:700;color:var(--l-ink);font-size:15px;">📅 ${T('topt.day', 'Giorno')} ${day.dayIdx + 1}</span>
            <span style="font-size:14px;color:${freeColor};font-weight:600;">⏳ ${_fmtFree(free)} ${T('sugg.free', 'liberi')}</span>
          </div>
          ${suggList}
        </div>`;
    }).join('');
