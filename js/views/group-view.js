  ...
  function renderGroupView() {
    ...
    const html = `
      <div style="padding:12px;display:flex;flex-direction:column;gap:12px;box-sizing:border-box;">
        ...
        <div style="display:flex;gap:8px;">
          <button id="mode-create" class="btn-plain" style="flex:1;padding:10px;background-image:linear-gradient(180deg,var(--l-accent),var(--l-accent-600));border:2px solid var(--l-accent-600);color:white;border-radius:8px;font-weight:600;cursor:pointer;font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,sans-serif;box-sizing:border-box;">
            ➕ Crea Nuova
          </button>
          <button id="mode-join" class="btn-plain" style="flex:1;padding:10px;background:var(--l-btn-bg);border:2px solid var(--l-hair);color:var(--l-ink);border-radius:8px;font-weight:600;cursor:pointer;box-sizing:border-box;">
            🔓 Entra Esistente
          </button>
        </div>
        ...
        <div id="create-mode" style="display:block">
          ...
          <div style="font-size:13px;color:var(--l-muted);margin-top:6px;">Condividi questo codice con chi vuoi far entrare. Solo chi lo ha può entrare.</div>
        </div>
        ...
        <!-- COMMON FIELDS -->
        <div>
          <label style="display:block;margin-bottom:4px;font-size:15px;font-weight:600;color:var(--l-ink)">Il tuo nome</label>
          <input id="group-name" placeholder="Es. Marco" style="width:100%;padding:10px;border:2px solid var(--l-hair);background:var(--l-input-bg);color:var(--l-ink);border-radius:8px;box-sizing:border-box;font-size:15px;" value="${state.group?.myName||''}">
        </div>
        <div>
          <label style="display:block;margin-bottom:4px;font-size:15px;font-weight:600;color:var(--l-ink)">Avatar profilo (opzionale)</label>
          <input id="group-avatar" type="text" placeholder="URL immagine (es. https://example.com/avatar.png)" style="width:100%;padding:10px;border:2px solid var(--l-hair);background:var(--l-input-bg);color:var(--l-ink);border-radius:8px;box-sizing:border-box;font-size:15px;margin-bottom:6px;" value="${state.group?.myAvatar?.startsWith('data:') || state.group?.myAvatar?.startsWith('http') ? state.group.myAvatar.substring(0,50) + '...' : state.group?.myAvatar || ''}">
          <button id="gallery-upload-btn" class="btn-plain" style="width:100%;padding:10px;background:var(--l-btn-bg);border:2px solid var(--m-success);color:var(--l-ink);border-radius:6px;font-weight:600;cursor:pointer;margin-bottom:6px;box-sizing:border-box;font-size:15px;">
            🎨 Scegli dalla Galleria
          </button>
          <input id="avatar-file-input" type="file" accept="image/*" style="display:none">
          <div style="font-size:13px;color:var(--l-faint);">Lascia vuoto per generare un avatar con le tue iniziali.</div>
        </div>

        <button id="group-start" class="btn-plain" style="width:100%;padding:12px;background-image:linear-gradient(180deg,var(--l-accent),var(--l-accent-600));color:white;border:2px solid var(--l-accent-600);border-radius:8px;font-weight:600;cursor:pointer;font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,sans-serif;box-sizing:border-box;font-size:16px;">✅ Connetti</button>
        <div id="peer-status-box" style="color:var(--l-faint);font-size:15px;">Pronto a connettersi...</div>
      </div>
    `;
    ...
    if (modeCreateBtn && modeJoinBtn) {
      modeCreateBtn.onclick = () => {
        ...
        modeCreateBtn.style.backgroundImage = 'linear-gradient(180deg,var(--l-accent),var(--l-accent-600))';
        modeCreateBtn.style.borderColor = 'var(--l-accent-600)'; modeCreateBtn.style.color = 'white';
        modeJoinBtn.style.background = 'var(--l-btn-bg)'; modeJoinBtn.style.borderColor = 'var(--l-hair)';
        modeJoinBtn.style.color = 'var(--l-ink)';
      };
      modeJoinBtn.onclick = () => {
        createModeDiv.style.display = 'none'; joinModeDiv.style.display = 'block';
        modeJoinBtn.style.backgroundImage = 'linear-gradient(180deg,var(--l-accent),var(--l-accent-600))';
        modeJoinBtn.style.borderColor = 'var(--l-accent-600)'; modeJoinBtn.style.color = 'white';
        modeCreateBtn.style.background = 'var(--l-btn-bg)'; modeCreateBtn.style.borderColor = 'var(--l-hair)';
        modeCreateBtn.style.color = 'var(--l-ink)';
      };
    }
