...
(function () {
  ...
  function _broadcast(action, data) {
    ...
    window.rtdbBroadcast?.({ type: 'group_expense', payload });
  ...
  function openPanel() {
    ...
    const balRows = Object.entries(bal).sort((a, b) => b[1] - a[1]).map(([m, v]) => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:#fff;font-size:14px;font-weight:600;">${_esc(m)}${m === me ? ' <span style="font-size:10px;color:rgba(255,255,255,0.5)">(tu)</span>' : ''}</span>
        <span style="font-weight:700;color:${v > 1 ? '#4ade80' : v < -1 ? '#f87171' : 'rgba(255,255,255,0.5)'};">${v > 1 ? '+' : ''}${_yen(v)}</span>
      </div>`).join('');

    const settRows = setts.length ? setts.map(s => `
      <div style="display:flex;align-items:center;gap:8px;padding:10px 12px;background:rgba(255,107,53,0.1);border:1px solid rgba(255,107,53,0.25);border-radius:8px;margin-bottom:6px;">
        <span style="color:#f87171;font-weight:700;">${_esc(s.from)}</span>
        <span style="color:rgba(255,255,255,0.6);">→</span>
        <span style="color:#4ade80;font-weight:700;">${_esc(s.to)}</span>
        <span style="margin-left:auto;color:#fff;font-weight:800;">${_yen(s.amount)}</span>
      </div>`).join('') : `<p style="color:rgba(255,255,255,0.5);font-size:13px;text-align:center;padding:10px;">${T('exp.allSettled', '✅ Tutti pari, nessun rimborso')}</p>`;

    const expRows = _list().slice().reverse().map(e => `
      <div style="display:flex;align-items:center;gap:8px;padding:10px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;margin-bottom:6px;">
        <div style="flex:1;min-width:0;">
          <div style="color:#fff;font-size:13px;font-weight:600;">${_esc(e.desc)}</div>
          <div style="color:rgba(255,255,255,0.5);font-size:11px;">${T('exp.paidBy', 'pagato da')} ${_esc(e.paidBy)} · ${T('exp.splitN', 'diviso tra')} ${e.splitAmong.length}</div>
        </div>
        <div style="color:#fff;font-weight:700;font-size:14px;">${_yen(e.amount)}</div>
        ${(e.by === me || e.paidBy === me) ? `<button onclick="window.GroupExpenses.remove('${_esc(e.id)}')" style="background:none;border:none;color:rgba(255,255,255,0.4);cursor:pointer;font-size:14px;padding:4px;">🗑️</button>` : ''}
      </div>`).join('');
      ...
      <div id="group-exp-body" style="padding:4px 2px;">
        ...
        <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:14px;margin-bottom:12px;">
          <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
            ...
            <span style="color:rgba(255,255,255,0.6);font-size:12px;">${T('exp.total', 'Totale')}: ${_yen(total)}</span>
          </div>
          ${balRows || `<p style="color:rgba(255,255,255,0.5);font-size:13px;">${T('exp.noMembers', 'Nessun membro')}</p>`}
        </div>
        ...
        <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:14px;margin-bottom:12px;">
          ...
          <input id="exp-desc" type="text" placeholder="${T('exp.descPh', 'Es: cena ramen GF')}" style="width:100%;padding:9px;margin-bottom:8px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);border-radius:7px;color:#fff;font-size:13px;box-sizing:border-box;">
          <div style="display:flex;gap:8px;margin-bottom:8px;">
            <input id="exp-amount" type="number" min="0" placeholder="¥0" style="flex:1;padding:9px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);border-radius:7px;color:#fff;font-size:13px;box-sizing:border-box;">
            <select id="exp-paidby" style="flex:1;padding:9px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);border-radius:7px;color:#fff;font-size:13px;">${memberOpts}</select>
          </div>
          <button id="exp-add-btn" style="width:100%;padding:10px;background:rgba(255,107,53,0.25);border:1.5px solid rgba(255,107,53,0.45);border-radius:8px;color:#fff;font-size:14px;font-weight:700;cursor:pointer;">${T('exp.addBtn', '+ Registra spesa')}</button>
          <p style="color:rgba(255,255,255,0.45);font-size:11px;margin:8px 0 0;">${T('exp.splitHint', 'Diviso equamente tra tutti i membri del gruppo.')}</p>
        </div>

        <!-- Lista spese -->
        ${_list().length ? `<div style="margin-top:6px;"><h3 style="margin:0 0 8px;color:rgba(255,255,255,0.7);font-size:13px;">${T('exp.history', 'Spese')}</h3>${expRows}</div>` : ''}
      </div>`;
