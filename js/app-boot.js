...
// ── Themed modal dialogs (replace native alert/confirm/prompt) ────────────────
// window.modalConfirm(msg, opts)  → Promise<boolean>
// window.modalPrompt(msg, opts)   → Promise<string|null>
(function() {
  function _overlay() {
    const el = document.createElement('div');
    el.style.cssText = 'position:fixed;inset:0;z-index:19999;background:rgba(0,0,0,0.65);display:flex;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(4px)';
    return el;
  }
  function _box() {
    const el = document.createElement('div');
    el.style.cssText = 'background:#1a1f2b;border:1px solid rgba(255,255,255,0.12);border-radius:14px;padding:20px 20px 16px;max-width:320px;width:100%;box-shadow:0 8px 32px rgba(0,0,0,0.5);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';
    return el;
  }
  function _msg(text) {
    const el = document.createElement('p');
    el.style.cssText = 'color:rgba(255,255,255,0.9);font-size:14px;line-height:1.5;margin:0 0 16px';
    el.textContent = text;
    return el;
  }
  function _btn(label, primary, danger) {
    const el = document.createElement('button');
    el.textContent = label;
    const bg = danger ? 'rgba(239,68,68,0.85)' : primary ? 'var(--m-accent,#ff7a45)' : 'rgba(255,255,255,0.08)';
    const border = danger ? 'rgba(239,68,68,0.5)' : primary ? 'transparent' : 'rgba(255,255,255,0.14)';
    el.style.cssText = `flex:1;padding:10px 14px;background:${bg};border:1px solid ${border};border-radius:8px;color:#fff;font-size:13px;font-weight:600;cursor:pointer;min-height:44px`;
    return el;
  }
  ...
  window.modalConfirm = function(message, opts) {
    ...
    return new Promise(function(resolve) {
      const ov = _overlay();
      const box = _box();
      box.appendChild(_msg(message));
      const row = document.createElement('div');
      row.style.cssText = 'display:flex;gap:8px';
      const cancel = _btn(opts.cancelText || (window.t && window.t('common.cancel')) || 'Annulla', false, false);
      const ok = _btn(opts.confirmText || (window.t && window.t('common.ok')) || 'OK', !opts.danger, !!opts.danger);
      cancel.onclick = function() { ov.remove(); resolve(false); };
      ok.onclick     = function() { ov.remove(); resolve(true); };
  ...
  window.modalPrompt = function(message, opts) {
    ...
    return new Promise(function(resolve) {
      ...
      input.style.cssText = 'width:100%;padding:10px 12px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);border-radius:8px;color:#fff;font-size:14px;margin-bottom:14px;box-sizing:border-box;outline:none';
      box.appendChild(input);
      const row = document.createElement('div');
      row.style.cssText = 'display:flex;gap:8px';
      const cancel = _btn((window.t && window.t('common.cancel')) || 'Annulla', false, false);
      const ok = _btn((window.t && window.t('common.ok')) || 'OK', true, false);
      cancel.onclick = function() { ov.remove(); resolve(null); };
      ok.onclick     = function() { ov.remove(); resolve(input.value); };
      input.onkeydown = function(e) { if (e.key === 'Enter') ok.click(); if (e.key === 'Escape') cancel.click(); };
      row.appendChild(cancel);
      row.appendChild(ok);
      box.appendChild(row);
      ov.appendChild(box);
